...
#pragma endregion round関数
	...
	{
		smoothTime = EngineMathUtility::Max(0.0001f, smoothTime);
	...
	{
		float tmp = 1e-06f * EngineMathUtility::Max(Abs(a), Abs(b));

		float tmp2 = EPSILON * 8.0f;

		if (Abs(b - a) < EngineMathUtility::Max(tmp, tmp2))
		{
			return true;
		}
	...
	float LenSegOnSeparateAxis(Vector3* Sep, Vector3* e1, Vector3* e2, Vector3* e3)
	{
	   ...
	   // 分離軸Sepは標準化されていること
		float r1 = EngineMathF::Abs(Sep->dot(*e1));
		float r2 = EngineMathF::Abs(Sep->dot(*e2));
		float r3 = e3 ? (EngineMathF::Abs(Sep->dot(*e3))) : 0;
		return r1 + r2 + r3;
	}
}